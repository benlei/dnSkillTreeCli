...
public class DDSImageReader extends ImageReader {
    ...
    private long skipImage(DDSLineReader ddsLineReader, int index) {
        long skipsBytes = 0;
        if (ddsHeader.getFormat() != DDSFormat.UNCOMPRESSED) {
            ...
            if (ddsHeader.getFormat() == DDSFormat.DXT1 || ddsHeader.getFormat() == DDSFormat.ATI1) { //DXT1 & ATI (8 bytes)
    ...
    private void readHeader() throws IIOException {
        ...
        try {
            ...
            if (ddsPixelFormat.getFormat() == DDSFormat.DX10) {
